...
type Draft = { start: Point; current: Point } | null;
...
export function EditorCanvas() {
  ...
  const onCanvasPointerDown = async (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (["line", "rectangle", "circle", "connector"].includes(tool)) {
      event.currentTarget.setPointerCapture(event.pointerId);
  ...
  const onPointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (drag) setDrag({ ...drag, current: point });
  };
  ...
  const onPointerUp = async (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (tool === "line" && (width || height)) {
      ...
      const midX = start.x + (current.x - start.x) / 2;
      await addElement(
        {
          type: "connector",
          points: [start, { x: midX, y: start.y }, { x: midX, y: current.y }, current],
          process_tag: "",
        },
  ...
  const renderedElements = document.elements.map((element) => {
    const active = drag?.element.id === element.id
      ? translateElement(element, drag.current.x - drag.start.x, drag.current.y - drag.start.y)
      : element;
    return (
      <g
        key={element.id}
      ...
      >
        {renderElement(active, symbolMap)}
      </g>
    );
  });
...
function DraftPreview({ tool, start, current }: { tool: string; start: Point; current: Point }) {
  ...
  if (tool === "connector") {
    const midX = start.x + (current.x - start.x) / 2;
    return <polyline points={`${start.x},${start.y} ${midX},${start.y} ${midX},${current.y} ${current.x},${current.y}`} {...common} />;
  }
  if (tool === "rectangle") {
