...
type RightPanel = "properties" | "agent";
...
export default function App() {
  ...
  useEffect(() => {
    if (state.selectedElementIds.length) setRightPanel("properties");
      ...
      <main className="workspace">
        ...
        <aside className="sidebar right-panel">
          <div className="right-panel-tabs" role="tablist" aria-label="右侧面板">
            ...
            >
              属性
              {state.selectedElementIds.length ? <span>{state.selectedElementIds.length}</span> : null}
            </button>
            <button
              type="button"
          ...
          {rightPanel === "properties" ? (
            ...
            <section className="agent-panel" role="tabpanel">
              <h2>P&amp;ID Agent</h2>
              <label>
                工艺/设计上下文
                <textarea
