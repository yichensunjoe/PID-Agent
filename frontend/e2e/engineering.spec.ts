...
test("creates a document, places two devices, connects real ports and preserves binding after drag", async ({ page, request }) => {
  ...
  const tankDialog = page.getByRole("dialog", { name: /放置/ });
  await expect(tankDialog).toBeVisible();
  await tankDialog.getByRole("button", { name: "确认并放置" }).click();
  ...
  const pumpDialog = page.getByRole("dialog", { name: /放置/ });
  await expect(pumpDialog).toBeVisible();
  await pumpDialog.getByRole("button", { name: "确认并放置" }).click();
