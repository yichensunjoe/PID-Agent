...
def main() -> None:
    try:
        ...
        raise SystemExit("Install MCP support with: pip install 'agentcad[mcp]'") from exc

    service = build_service()
    mcp = FastMCP("AgentCAD")

    @mcp.tool()
    def list_documents() -> list[dict]:
        """List AgentCAD documents and their current revisions."""
    ...
    def get_document(document_id: str) -> dict:
        """Read the complete current AgentCAD document JSON."""
    ...
    def apply_transaction(document_id: str, transaction_json: str) -> dict:
        """Apply one atomic AgentCAD transaction. transaction_json must match the v2 TransactionRequest schema."""
    ...
    mcp.run(transport=os.getenv("AGENTCAD_MCP_TRANSPORT", "stdio"))
