...
<form id="form"><label class="wide">Base URL<input id="base" value="https://apihub.agnes-ai.com/v1" required></label><label>Model<input id="model" value="agnes-2.0-flash" required></label><label>API Key<input id="key" type="password" autocomplete="off"></label><label>超时（秒）<input id="timeout" type="number" min="10" max="600" value="120"></label><label>重复次数<input id="repetitions" type="number" min="1" max="5" value="3"></label><label>最大重规划次数<input id="replans" type="number" min="0" max="5" value="3"></label><label class="wide check"><input id="complex" type="checkbox">增加复杂整图场景（主管、四个仪表测点、第二介质线路和标签碰撞检查）</label><div class="wide actions"><button id="run" type="submit">运行模型矩阵</button><button id="download" type="button" disabled>下载 JSON 报告</button><strong id="status"></strong></div></form>
